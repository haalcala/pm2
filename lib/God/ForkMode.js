...
module.exports = function ForkMode(God) {
  ...
  God.forkMode = function forkMode(pm2_env, cb) {
    ...
    if (pm2_env.PM2_ALERT_MODE == "aws-sns") {
      if (!alert_sender.configured) {
        alert_sender.configure({
          default_channel: "sns",
          sns: {
            "type"				: "sns",
            "accessKeyId"   : pm2_env.PM2_ALERT_AWS_SNS_KEY,
            "secretAccessKey" : pm2_env.PM2_ALERT_AWS_SNS_SECRET,
            "region"      : pm2_env.PM2_ALERT_AWS_SNS_REGION,
            "topic"				: pm2_env.PM2_ALERT_AWS_SNS_ARN
          },
    ...
    Utility.startLogging(stds, function(err, result) {
      ...
      var error_data = "", no_exit_timeout = 1000, error_log_timer_id;

      cspr.stderr.on('data', function forkErrData(data) {
        var log_data = null;

        if (alert_sender.configured && log_data != null) {
          error_data += log_data;

          console.log("error_data '--------" + error_data + "----------'");
          
          if (error_log_timer_id) {
            clearTimeout(error_log_timer_id);
          }
  
          error_log_timer_id = setTimeout(function() {
            if (error_data) {
              alert_sender.send(
                "Script failure from stderr: pm2_env.pm_exec_path: " + pm2_env.pm_exec_path + " " + (args && args.length > 0 ? args.join(" ") : "") + "\nHostname: " + os.hostname() + "\n\n" + error_data,
                { subject: os.hostname() + " Script failure from stderr" }
              );
            }
          }, no_exit_timeout);
        }
      ...
      cspr.once('exit', function forkClose(status, code) {
        console.log("error_data '--------" + error_data + "----------'");
        if (alert_sender.configured && error_data) {
          alert_sender.send(
            "Script failure status: " + status + " code: " + code + " pm2_env.pm_exec_path: " + pm2_env.pm_exec_path + " " + (args && args.length > 0 ? args.join(" ") : "") + "\nHostname: " + os.hostname() + "\n\n" + error_data,
            { subject: os.hostname() + " Script failure status: " + status }
          );
